...
class API:
    ...
    def createNewPaste(self,string,name):
        ...
        if(not '201' in str(r)): raise Exception('Unable to process the requests: '+str(r))

    def retrieveLastId(self):
        headers = {'X-Auth-Token':self.token}
        id = self.getLastPaste()
        r = requests.get('https://api.paste.ee/v1/pastes/'+id,headers = headers)
        if(not '200' in str(r)): raise Exception('Unable to process the requests: '+str(r))
